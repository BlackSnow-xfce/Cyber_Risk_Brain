...
EXTRA = {
 "product-owner-recovery-decision": {"principal","operation","approval_context_id","approval_context_digest","decision_id","decision_digest","nonce","selected_source_digest","selected_source_authority_id"},
 ...
 "execution-source-authority": {"selected_source_authority_id","selected_source_digest","authority_digest","authority_terms_digest","authority_lifecycle_state"},
...
class AttestationBundleVerifier:
    ...
    def verify(self, attestations: list[bytes], *, policies: dict[str, Any], requests: dict[str, dict[str, Any]], environment: str, audience: str, endpoint_identities: dict[str, str], trust_store: dict[str, Any], revoked_key_ids: set[str], public_keys: dict[str, bytes], payload_schema: str, now: str, manifest: AttestationBundleManifestV1 | None = None) -> tuple[AttestationBundleManifestV1, tuple[SourceAttestation, ...]]:
        ...
        p=po_item.payload; po_payload={"authenticated_principal_ref":p["principal"],**{k:p[k] for k in ("approval_context_digest","decision_id","decision_digest","nonce","proposal_digest","predecessor_authority_id","predecessor_claim_digest","predecessor_execution_id","dependency_id","parent_task_id","selected_source_authority_id","selected_source_digest","issued_at","valid_until")}}; po_payload.update(schema_version="aidp-product-owner-recovery-decision-v1", domain="aidp-product-owner-recovery-decision", permission="RECOVER_GATE_DEPENDENCY", approval_context_ref=p["approval_context_id"])
        s=source_item.payload; source_payload={"schema_version":"aidp-execution-source-authority-v1","domain":"aidp-execution-source-authority","authority_id":s["selected_source_authority_id"],"authority_digest":s["authority_digest"],"terms_digest":s["authority_terms_digest"],"lifecycle":s["authority_lifecycle_state"],"proposal_digest":po_payload["proposal_digest"],"po_decision_id":po_payload["decision_id"],"po_decision_digest":po_payload["decision_digest"],"selected_source_authority_id":s["selected_source_authority_id"],"selected_source_digest":s["selected_source_digest"]}
