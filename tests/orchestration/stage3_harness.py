...
class _Allow:
    def verify_recovery_decision(self, value):
        return AuthoritativeDecisionRecord({"schema_version":"aidp-authoritative-po-v1","principal":value["authenticated_principal_ref"],"approval_context_id":value["approval_context_ref"],**{k:v for k,v in value.items() if k not in ("authenticated_principal_ref","approval_context_ref","schema_version","domain")}})
    def verify_source_authority(self, value):
        return AuthoritativeSourceRecord({"schema_version":"aidp-authoritative-source-v1","terms_digest":value["terms_digest"],"lifecycle":value["lifecycle"],"issued_at":"2026-09-11T12:00:00.000000Z","valid_until":"2026-09-11T13:00:00.000000Z",**{k:v for k,v in value.items() if k not in ("schema_version","domain","terms_digest","lifecycle")}})
...
class Stage3TestTrustHarness:
    ...
    def create(cls, root: Path):
        ...
        lineage={"dependency_id":"dep-test","parent_task_id":"parent-test","predecessor_authority_id":"pred-test","predecessor_claim_digest":"c"*64,"predecessor_execution_id":"exec-test","proposal_digest":"d"*64,"selected_source_authority_id":"source-test"}
    ...
    def build(self, category):
        ...
        if category == "product-owner-recovery-decision": payload.update(principal="po-test", operation="RECOVER_GATE_DEPENDENCY", approval_context_id="ctx", approval_context_digest="a"*64, decision_id="decision", decision_digest="b"*64, nonce="nonce", selected_source_authority_id="source-test", selected_source_digest="b"*64)
        ...
        if category == "execution-source-authority": payload.update(selected_source_authority_id="source-test", selected_source_digest="b"*64, authority_digest="b"*64, authority_terms_digest="2"*64, authority_lifecycle_state="ELIGIBLE")
